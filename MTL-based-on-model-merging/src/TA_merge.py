...
if __name__ == '__main__':
    ...
    for model in models:
        ...
        log = create_log_dir(args.logs_path, 'log_{}_TA_merge.txt'.format(str_time_))
        args.log = log
        ...
        #保存模型在预训练模型上的精度
        for dataset in datasets:
            fintuned_checkpoint = f"checkpoints/{model}/{dataset}/finetuned.pt"
            fintuned_model = torch.load(fintuned_checkpoint)
            metrics = eval_single_dataset(fintuned_model, dataset, args)
            fintuned_metrics.append(metrics['top1'])
            log.info('fintuned_model in dataset: ' + str(dataset) + ' ACC: ' + str(metrics['top1']))
        ...
        for scaling_coef in np.arange(0, 1.05, 0.05):
