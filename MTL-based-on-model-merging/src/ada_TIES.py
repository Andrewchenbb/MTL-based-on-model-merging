...
if __name__=="__main__":
    ...
    for model in models:
        ...
        log = create_log_dir(args.logs_path, 'log_{}_ada_TIES.txt'.format(str_time_))
        args.log = log

        #########################################################3
        ft_checks = [
            torch.load('checkpoints/' + model + '/' + dataset_name + '/finetuned.pt').state_dict() for
        ...
        for vector_ in selected_entries:
            ...
            ties_taszk_vectors.append(ref_model.state_dict())
