...
from eval import eval_single_dataset, eval_single_dataset_preprocess_head
...
class AdaMerging(torch.nn.Module):
    def __init__(self, paramslist, model, names, exam_datasets):
        ...
        prior = 0.5
        values = [0.6290, 0.6611, 0.4585, 0.4956, 0.4097, 0.7292, 0.3193, 0.6399]
...
if __name__=='__main__':
    exam_datasets = ['SUN397', 'Cars', 'RESISC45', 'EuroSAT', 'SVHN', 'GTSRB', 'MNIST',
                     'DTD']  # SUN397 | Cars | RESISC45 | EuroSAT | SVHN | GTSRB | MNIST | DTD
    model = 'ViT-L-14'
    ...
    args.log = log
    ft_checks = [torch.load('checkpoints/' + model + '/' + dataset_name + '/finetuned.pt').state_dict() for dataset_name
                 in exam_datasets]
    ...
    assert all([check_state_dicts_equal(vector_to_state_dict(flat_ft[i], ptm_check, remove_keys), ft_checks[i]) for i in
                range(len(ft_checks))])
    ...
    paramslist += [tuple(v.detach().requires_grad_().cpu() for _, v in tv.items()) for i, tv in
                   enumerate(ties_task_vectors)]  # task vectors
    ...
    # epochs = 500
    # optimizer = torch.optim.Adam(adamerging_mtl_model.collect_trainable_params(), lr=1e-3, betas=(0.9, 0.999),
    #                              weight_decay=0.)
    ...
    with torch.no_grad():
        ...
        for dataset_name in exam_datasets:
            ...
            Total_ACC += metrics['top1']
            log.info(
                 ' dataset: ' + str(dataset_name) + ' ACC: ' + str(metrics['top1']))
